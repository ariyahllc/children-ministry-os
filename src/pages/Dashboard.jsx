...
import {
  Users, GraduationCap, UserCheck, Calendar, CheckSquare,
...
export default function Dashboard() {
  ...
  // Build attendance trend data
  const attendanceByDate = {};
  attendance.forEach(a => {
    if (!a.date) return;
    const d = a.date;
    if (!attendanceByDate[d]) attendanceByDate[d] = { date: d, present: 0, absent: 0 };
    if (a.present) attendanceByDate[d].present++;
    else attendanceByDate[d].absent++;
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Dashboard" subtitle="Ministry overview at a glance" />

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Children" value={activeChildren} icon={Users} color="indigo" subtitle={`${children.length} total`} index={0} />
        <StatCard title="Teachers" value={activeTeachers} icon={GraduationCap} color="green" subtitle="Active" index={1} />
        <StatCard title="Volunteers" value={volunteers.length} icon={UserCheck} color="orange" subtitle="Registered" index={2} />
        <StatCard title="Upcoming Events" value={upcomingEvents.length} icon={Calendar} color="purple" index={3} />
      </div>
          ...
          <Card className="p-6 border-0 shadow-sm">
            ...
            <div className="space-y-2">
              {pendingTasks.slice(0, 6).map(task => (
                <div key={task.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted/30 transition-colors">
          ...
          <Card className="p-6 border-0 shadow-sm">
            ...
            <div className="space-y-2">
              {documents.slice(0, 6).map(doc => (
                <div key={doc.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors">
                  ...
                  {doc.created_date && (
                    <p className="text-xs text-muted-foreground">{format(new Date(doc.created_date), 'MMM d')}</p>
                  )}
                </div>
              ))}
              {documents.length === 0 && (
